...
public class TomTomDecoder {
  ...
  /**
   * Decodes byte array and outputs edges of the own routing network affected by
   * the incident.
   *
  ...
  public Location decodeTomTom(ByteArray byteArray,
                               RoutableOSMMapLoader osmMapLoader)
      throws Exception {
    // Byte array to location reference
    LocationReference lr =
        new LocationReferenceBinaryImpl("Incident", byteArray);
    ...
    FileConfiguration decoderConfig =
        OpenLRPropertiesReader.loadPropertiesFromFile(
            new File(this.getClass()
                         .getClassLoader()
                         .getResource("OpenLR-Decoder-Properties.xml")
                         .getFile()));
