...
public class LineImpl implements Line {
  ...
  public LineImpl(long line_id, long startNode_id, long endNode_id, int frc,
                  int fow, int length_meter, String name, boolean isReversed) {
  ...
  public void setMdb(MapDatabaseImpl mdb) { this.mdb = mdb; }
  ...
  public Point2D.Double getPointAlongLine(int distanceAlong) {
    ...
    if (distanceAlong < length_meter) {
      ...
      for (int i = 0; i <= lineCoordinates.length - 2; i++) {
        LineSegment segment =
            new LineSegment(lineCoordinates[i], lineCoordinates[i + 1]);
        // Get segment length
        Coordinate[] segmentCoordinates =
            new Coordinate[] {segment.p0, segment.p1};
        int segmentLength =
            (int)Math.round(calculateOthodromicDist(segmentCoordinates));
        segmentLengthTotal += segmentLength;

        if (segmentLengthTotal >= distanceAlong) {

          int newDistAlong =
              segmentLength - (segmentLengthTotal - distanceAlong);
          Coordinate pointAlong =
              segment.pointAlong((double)newDistAlong / (double)segmentLength);
  ...
  public GeoCoordinates getGeoCoordinateAlongLine(int distanceAlong) {
    ...
    try {
      coordinatesAlongLine =
          new GeoCoordinatesImpl(pointAlongLine.getX(), pointAlongLine.getY());
  ...
  public int measureAlongLine(double longitude, double latitude) {
    ...
    Point p = geometryFactory.createPoint(new Coordinate(longitude, latitude));
    Point projectionPoint = geometryFactory.createPoint(
        DistanceOp.nearestPoints(lineGeometry, p)[0]);

    // Create Buffer around point geometry since jts is not able to check if
    ...
    for (int i = 1; i < lineCoordinates.length; i++) {
      LineString lineSegment = geometryFactory.createLineString(
          new Coordinate[] {lineCoordinates[i - 1], lineCoordinates[i]});

      if (lineSegment.intersects(pointBuffer)) {

        // create new currentLine with coordinateOnTheLine as endpoint and
        // calculate length
        LineString lineFraction =
            geometryFactory.createLineString(new Coordinate[] {
                lineCoordinates[i - 1], projectionPoint.getCoordinate()});
  ...
  @Override
  public int hashCode() {
    return Objects.hash(line_id, startNode_id, endNode_id, startNode, endNode,
                        frc, fow, length_meter, name, isReversed, lineGeometry);
  }
}
